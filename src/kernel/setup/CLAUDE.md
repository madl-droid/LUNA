...
- `templates.ts` — SSR templates del wizard (4 pasos), CSS inline, `SetupState` type
- `handler.ts` — HTTP handler del wizard: GET/POST /setup/step/{1-4}, validacion, finalizacion
...
3. Wizard 4 pasos: idioma -> admin -> LLM -> sistema
4. Al finalizar: crea tablas users, admin, credentials, config en config_store, SETUP_COMPLETED=true
