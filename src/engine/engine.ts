  ...
  try {
    ...
    const avisoTimer = avisoConfig.triggerMs > 0 && avisoConfig.messages.length > 0
      ? setTimeout(() => {
          ...
          sendAviso(ctx, pickAviso(avisoConfig.messages, avisoConfig.style, ctx.message.channelName), registry).catch(err =>
 ...
 */
function getAvisoConfig(channel: string): { triggerMs: number; holdMs: number; messages: string[]; style: import('../channels/types.js').AvisoStyle } {
  ...
  if (channelSvc) {
    ...
    return { triggerMs: cc.avisoTriggerMs, holdMs: cc.avisoHoldMs, messages: cc.avisoMessages, style: cc.avisoStyle }
  }
  // No channel-config service → aviso disabled for this channel
  return { triggerMs: 0, holdMs: 0, messages: [], style: 'formal' }
}

/**
 * Picks an aviso message from the pool based on style.
 * - formal/casual/express: random pick from pool
 * - dynamic: round-robin rotation through the pool (no randomness)
 * Messages are pre-defined per style, never generated by LLM.
 */
const avisoRotationIndex = new Map<string, number>()

function pickAviso(messages: string[], style: import('../channels/types.js').AvisoStyle, channel: string): string {
  if (messages.length === 0) return ''
  if (messages.length === 1) return messages[0]!

  if (style === 'dynamic') {
    // Round-robin: each channel gets its own rotation counter
    const key = `aviso:${channel}`
    const idx = avisoRotationIndex.get(key) ?? 0
    const msg = messages[idx % messages.length]!
    avisoRotationIndex.set(key, idx + 1)
    return msg
  }

  // All other styles: random pick from pool
