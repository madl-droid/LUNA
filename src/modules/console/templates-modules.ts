...
export function renderModulePanels(modules: ModuleInfo[], config: Record<string, string>, lang: Lang, filterName?: string): string {
  ...
  for (const mod of displayModules) {
    ...
    // Core agent modules that must always stay active — no toggle
    const ALWAYS_ACTIVE = new Set(['knowledge', 'memory', 'subagents'])
    const canToggle = mod.removable !== false && !filterName && !ALWAYS_ACTIVE.has(mod.name)
