...
// ─── Tool calling content blocks (native multi-turn format) ─────────────────

/** Plain text block in a multi-part message */
export interface TextBlock {
  type: 'text'
  text: string
}

/** Tool call block emitted by the LLM in the assistant turn */
export interface ToolUseBlock {
  type: 'tool_use'
  /** ID generated by the LLM (Anthropic: 'toolu_xxx', Google: synthetic 'gc_xxx') */
  id: string
  name: string
  input: Record<string, unknown>
}

/** Tool result block returned by LUNA in the user turn */
export interface ToolResultBlock {
  type: 'tool_result'
  /** Links back to ToolUseBlock.id */
  toolUseId: string
  /** Tool name — used by Google which matches by name instead of ID */
  name: string
  /** Serialized result (JSON string or plain text) */
  content: string
  /** True when the tool execution failed */
  isError?: boolean
}

/** Union of all content block types that can appear in a multi-part message */
...
export interface LLMToolCall {
  /** ID of the tool_use block. Anthropic provides it natively; Google gets a synthetic 'gc_xxx' ID. */
  id: string
  name: string
  input: Record<string, unknown>
}
